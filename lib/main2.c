...
int main (void){
    ...
    uint32_t left_threshold = 5; // Distance threshold to turn after passing object
...
uint32_t Sensor1_distance(){
    ...
    enum state_type state = send_trig;
    enum state_type next_state = state;
    ...
    while(!done){
        switch (state) {
            ...
            case echo_falling_edge:
                ...
                distance = time/148; // Distance from sensor in inches
                next_state = send_trig;
                done = 1;
...
void Pass_object(void){
    // passes 3in into drive straight so robot will go foward
    drive_straight(3); 
    timer_2us(1000000); 
    Stop_motors();
}
